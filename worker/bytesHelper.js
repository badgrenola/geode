...
function getDoubleFromBytes(bytes, byteOrder) {
    ...
    if (bytes.byteLength %4 !== 0) { console.error("Need multple of 4 bytes for a Double"); return null; }
...
function getUInt32sFromBytes(bytes, byteOrder) {
    //Ensure we have an even number of bytes
    if (bytes.byteLength %4 != 0) { console.error("Need an even number of bytes for a UInt32 array"); return null; }
    let offset = 0
...
function getDoublesFromBytes(bytes, byteOrder) {
    //Ensure we have an even number of bytes
    if (bytes.byteLength %8 != 0) { console.error("Need an even number of bytes for a UInt32 array"); return null; }
    let offset = 0
